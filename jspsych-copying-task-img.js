 ...
 *
 * a jsPsych plugin for displaying a copying task with catch trial initiation at midline crossing.
 *
 ...
 **/
 ...
 jsPsych.plugins["copying-task-img"] = (function () {

  var plugin = {};

  plugin.info = {
    name: 'copying-task-img',
    description: '',
    parameters: {
      items: {
      ...
      n_model_items: {
        ...
        pretty_name: 'Number of items in model',
        default: 6,
        description: 'The number of items in model to be copied'
      },
      catch_probability: {
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Catch Probability',
        default: -1,
        description: 'Probability of initiating catch trial at midline crossing to model'
      },
      item_scale: {
        type: jsPsych.plugins.parameterType.INT,
      ...
      grid_colors: {
        ...
        description: 'The colors of the grids as such [model color, workspace color, resource color] '
      },
      color_palette: {
        type: jsPsych.plugins.parameterType.STRING,
        array: true,
        pretty_name: 'Color Palette',
        default: ["rgb(247, 113, 137)", "rgb(247, 117, 79)", "rgb(220, 137, 50)", "rgb(195, 149, 50)", "rgb(174, 157, 49)", "rgb(151, 164, 49)", "rgb(119, 171, 49)", "rgb(49, 179, 62)", "rgb(51, 176, 122)", "rgb(53, 174, 147)", "rgb(54, 173, 164)", "rgb(55, 171, 180)", "rgb(56, 169, 197)", "rgb(58, 165, 223)", "rgb(110, 155, 244)", "rgb(164, 140, 244)", "rgb(204, 122, 244)", "rgb(244, 92, 242)", "rgb(245, 101, 204)", "rgb(246, 107, 173)"] ,
        description: 'The colors to choose from'
      },
      choices: {
      ...
      catch_on:{
        type: jsPsych.plugins.parameterType.INT,
        pretty_name: 'Catch position',
        default: null,
        description: 'Initiate catch trial after these number of correct item placements' 
      },
      catch_on_event: {
        type: jsPsych.plugins.parameterType.STRING,
        pretty_name: 'Catch on Event',
        default: 'crossed midline towards model', // could be either: 'crossed midline towards model' or 'crossed midline towards workspace'
        description: 'The event at which (the catch prob. taken into account) a catch trial might be initiated' 
      },
      grid_contents:{
        type: jsPsych.plugins.parameterType.OBJECT,
        pretty_name: 'Grid States',
        default: null,
        description: 'The state of the grids' 
      },
      twenty_svg_paths:{
        type: jsPsych.plugins.parameterType.STRING,
  ...
  plugin.trial = function (display_element, trial) {
    ...
    // create the html 
    var html = '<div id="jspsych-canvas-copying-task-img">' 
    ...
    // set the top and left positions of grids on the canvas
        // model grid
    var mdl_grid_top  = canvas_height * 0.5   - (trial.grid_rows * grid_box_size) * 0.5; // vertically centered
    var mdl_grid_left = canvas_width  * 0.01 ; // left edge of grid at 1% canvas width

        // workspace grid
    var wsp_grid_top  = canvas_height * 0.5  - (trial.grid_rows * grid_box_size) * 0.5; // vertically centered
    var wsp_grid_left = canvas_width  * 0.56 ; // left edge of grid at 56% canvas width

        // resource grid
    var rsc_grid_top  = canvas_height * 0.5  - (trial.grid_rows * grid_box_size) * 0.5; // vertically centered
    var rsc_grid_left = canvas_width  * 0.79; // left edge of grid at at 79% canvas width

    // make an array for each grid with (row * col) number of elements
    var n_grid_boxes = trial.grid_rows * trial.grid_cols;
    var model_grid     = new Array(n_grid_boxes); 
    var workspace_grid = new Array(n_grid_boxes);
    var resource_grid  = new Array(n_grid_boxes);

    // fill the grid (arrays) with box objects and values
    for (var row=0; row<trial.grid_rows; row++) {
      for (var col=0; col<trial.grid_cols; col++) {
 
        var box_index = getBoxIndex(row, col, trial.grid_cols)
        
        model_grid[box_index] = {
          index: box_index,
          content: null, 
        ...
        workspace_grid[box_index] = {
          index: box_index,
          content: null,
          correct_content: null,  // the workspace will need to store the correct item for each box
    ...
    //    make sure these two scalars are the same as in the mouseview library
    var mid_border_left  = canvas_width * 0.35;
    var mid_border_width = canvas_width * 0.17; 
    ...
    // set variables handling the catch trial initiations 
    var catch_trial_die_in_hand = true;

    var throw_die_on_event = trial.catch_on_event;
    var pick_up_die_on_event;

    if (throw_die_on_event == 'crossed midline towards model'){
      pick_up_die_on_event = 'crossed midline towards workspace';

    } else if (throw_die_on_event == 'crossed midline towards workspace') {
      pick_up_die_on_event = 'crossed midline towards model';
    }

        
    var interval_ID;
    interval_ID = setInterval( handleCatchInitiation , 10)
    ...
    if (trial.grid_contents === null) {

      // select unique locations from the grid topleft positions (sample WITHOUT repl.)
      var selected_model_boxes = jsPsych.randomization.sampleWithoutReplacement(model_grid, trial.n_model_items);
      
      // select items from the item pool: (sample WITH repl.) objects may appear multiple times
      var selected_model_items = jsPsych.randomization.sampleWithReplacement(trial.items, trial.n_model_items);

      for (var i=0; i < selected_model_items.length; i++)  {
        
        selected_model_boxes[i].content = selected_model_items[i];
        
        box_index = selected_model_boxes[i].index

        // set a correct item property on the workspace boxes
        workspace_grid[box_index].correct_content = selected_model_items[i]
        ...
        createItemFromURL(selected_model_boxes[i], options_object)
      };

    } else { // if continuing from previous trial
      for (var i=0; i < model_grid.length; i++) {
        // set the content of the current's trials model grid to the input
        model_grid[i].content = trial.grid_contents.model[i];
        
        // set a correct item property on the workspace boxes
        workspace_grid[i].correct_content = trial.grid_contents.model[i]

        if (trial.grid_contents.model[i] != null){
          ...
          createItemFromURL(model_grid[i], options_object)
        };
      }
    }      
    
    // populate the WORKSPACE GRID 
      
    if (trial.grid_contents === null) {
      // if starting a new trial nothing need to be done here
    } else {
      // if continuing from previous trial
      for (var i=0; i < workspace_grid.length; i++)  {
        
        // set the content of the current trials workspace grid to the input
        workspace_grid[i].content = trial.grid_contents.workspace[i];

        if (trial.grid_contents.workspace[i] != null) {
          ...
          // workspace_grid[i].content = trial.grid_contents.workspace[i],
          createItemFromURL(workspace_grid[i], options_object)
        }
      };
    }
    ...
    if (trial.grid_contents === null) { // starting a new trial
        
      for (var rep=0; rep<trial.n_model_items; rep++) {

        // loop over the resource grid boxes:  grid_obj[box]
        resource_grid.forEach( function(box) {

          // Set the content of the box
          box.content = trial.items[box.index];
          ...
          createItemFromURL(box, options_object)
        });
      }
      ...
      for (var rep=0; rep<trial.n_model_items; rep++) {
        ...
        resource_grid.forEach( function(box) {
          ...
          // Set the content of the box
          box.content = trial.grid_contents.resource[box.index];
    ...
    function createItemFromURL(box, options_object){
      ...
      fabric.Image.fromURL(box.content, function(item) { 
    ...
    function CreateItem(box, options_object){
      // get the item url and the color tag from the complete info  
      // var [item_url, item_color_tag,] = getURLandColorTag(box.content) 
      var [item_shape_num, item_color_tag] = getSVGPathandColorTag(box.content) 



      // fabric.Image.fromURL(ex_url, function(item) {
      //   canvas.add(item);
      // });
    ...
    function CreateItemOLD(box, options_object){
      // get the item url and the color tag from the complete info  
      var [item_url, item_color_tag] = getURLandColorTag(box.content) 

      // Create images form urls (file paths) 
      var group = [];
      fabric.loadSVGFromURL(item_url, function(objects, options){
        var item = new fabric.Group(group);
      ...
      }, function(item_path_el, object) { // second parameter is a function
        if (object.fill != "#fbfbfb"){
          object.set({fill: item_color_tag}) // color is set here
        } 
        group.push(object);
      });
    ...
    // Convenience functions to change between box-indices and their row/col coordinates (assumes indexing along rows aka horizontally)
    function getGridCoords(box_index, n_cols) {
      var row = Math.floor(box_index / n_cols);
      var col = box_index % n_cols
      return [row, col]
    }

    function getBoxIndex(row, col, n_cols) {
    ...
    function drawGridArray(grid){
      ...
      // loop over the boxes of the grid (note: the grid is an array and had boxes as elements, which are objects)
    ...
    function mouseDown(e){
      ...
      // register pick up event
      var event = {
        event: 'item was picked up',
        item: held_item_id,
    ...
    function droppedItem(item){
      if (held_item_id != null){
        ...
        if ( isInWorkspace(item_center_xy) ) {
          ...
          // loop over boxes
          workspace_grid.forEach(wsp_box => {
            // if the box is empty
            if (isInBox(item_center_xy, wsp_box)) { // if item is dropped in a box
              if (wsp_box.content == null){ // if box is empty

                
                if ( ( item.itemType == wsp_box.correct_content ) || (trial.allow_incorr_placement) ){ // if item in the corect box OR if we allow incorrect placement
                  //console.log(item)
                  ...
                  var n_correctly_placed_items = 0;
                  workspace_grid.forEach(wsp_box => {
                    if (wsp_box.content != null){
                      n_correctly_placed_items++
                    }
                  })

                  if (trial.catch_on == n_correctly_placed_items){
                    end_trial()
                  }

                } else { // if item is placed incorrectly AND we do not allow incorrect placement
                  send_back = true;
                  //console.log("incorrectly placed item")
                }

              } else { //if there is an item in that box already
                send_back = true;
                //console.log("box was full ")
              }
            } 
          });
        } else { // else if item is NOT dropped on the workspace
          send_back = true;
        }
      ...
      if ( checkIfDone() ){ // when done copying 
    ...
    function sendToResource(item){      
      ...
      } else { // if item is released near the workspace animate it moving back to its inital location
    ...
    function isInWorkspace (xy) {
      return ( 
         (xy[0] >= workspace_grid[0].left)  // left of upper-left most box
      && (xy[0] <  workspace_grid[0].left + trial.grid_cols * grid_box_size) 
      && (xy[1] >= workspace_grid[0].top)   // top  of upper-left most box
      && (xy[1] <  workspace_grid[0].top  + trial.grid_rows * grid_box_size)
          )
    }
    ...
    function checkIfDone(){
      ...
      for (var box_index = 0; box_index<model_grid.length; box_index++){
        // check if every box is correctly filled
        if ( model_grid[box_index].content != workspace_grid[box_index].content) {
    ...
    function getGridContent(grid){
      ...
      for (var  i=0; i<grid.length; i++){
        return_obj[i] = grid[i].content
      }
      return return_obj
    }

    function throw_die_condition(catch_on_event, latest_mouseview_event){
      if (catch_on_event == 'crossed midline towards model'){
        return (latest_mouseview_event.event == 'aperture: workspace -> quicksand')
      } else if (catch_on_event == 'crossed midline towards workspace') {
        return (latest_mouseview_event.event == 'aperture: model -> quicksand' || latest_mouseview_event.event == 'aperture: model -> workspace')
      }      
    }

    function pickup_die_condition(catch_on_event, latest_mouseview_event){
      if (catch_on_event == 'crossed midline towards model'){
        return (latest_mouseview_event.event == 'aperture: quicksand -> workspace' || latest_mouseview_event.event == 'aperture: model -> workspace')
      } else if (catch_on_event == 'crossed midline towards workspace') {
        return (latest_mouseview_event.event == 'aperture: workspace -> quicksand'|| latest_mouseview_event.event == 'aperture: quicksand -> model')
      }
    }
    

    function handleCatchInitiation() { // is triggered & run every 10 ms. So at least every frame with a 60 Hz refresh rate!

      var latest_mouseview_event =  mouseview.datalogger.trial_events[mouseview.datalogger.trial_events.length-1]
      
      if ( typeof(latest_mouseview_event)  != "undefined" ) {

        if (catch_trial_die_in_hand && throw_die_condition(trial.catch_on_event, latest_mouseview_event)){
          
          if ( Math.random() < trial.catch_probability ) { // draw a random float from the half open interval [0, 1) and compare to catch probability
            console.log( "Dice rolled: CATCH INITIATED! " )
            end_trial(); // end the trial and move to 
          } else{
            catch_trial_die_in_hand = false
            console.log( "Dice rolled: nothing happend " )
          }        
        }
        if ( (!catch_trial_die_in_hand) && pickup_die_condition(trial.catch_on_event, latest_mouseview_event)){
          catch_trial_die_in_hand = true
          console.log( "Dice back in hand " )
        }
      }   
    }

    // function getURLandColorTag(item_complete_info){
    //   var item_url = item_complete_info.slice(0,item_complete_info.length - 20);
    //   var item_color_tag = item_complete_info.slice(item_complete_info.length - 18, item_complete_info.length);

    //   return [item_url, item_color_tag]
    // }
    
    function getSVGPathandColorTag(item_complete_info){
      var item_shape_num = item_complete_info.slice(0,item_complete_info.length - 20);
      var item_color_tag = item_complete_info.slice(item_complete_info.length - 18, item_complete_info.length);

      return [item_shape_num, item_color_tag]
    }
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function () {
        display_element.querySelector('#jspsych-canvas-copying-task-img').style.visibility = 'hidden';
